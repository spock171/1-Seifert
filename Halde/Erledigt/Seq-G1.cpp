...
struct GtxModule : Module {
	...
	dsp::SchmittTrigger gateTriggers[BUT_ROWS][BUT_COLS];
	float phase  = 0.0f;
	int index    = 0;
	int numSteps = 0;
	...
	void process(const ProcessArgs& args) override
	{
		...
		prg_nob.step(params[PROG_PARAM].getValue() / 12.0f);
		...
		bool play_is_cv = (params[PLAY_PARAM].getValue() < 0.5f);
		bool edit_is_cv = (params[EDIT_PARAM].getValue() < 0.5f);
		...
		{
			const float dim = 1.0f / lightLambda * args.sampleTime;
		...
		numSteps = RATIO * clamp(roundf(params[STEPS_PARAM].getValue() + inputs[STEPS_INPUT].getVoltage()), 1.0f, static_cast<float>(LCD_COLS));
		...
		for (int col = 0; col < BUT_COLS; ++col)
		{
			for (int row = 0; row < BUT_ROWS; ++row)
			{
				...
				if (gateTriggers[row][col].process(params[but_map(row, col)].getValue()))
				{
					...
					std::size_t span_r = static_cast<std::size_t>(params[SPAN_R_PARAM].getValue() + 0.5f);
					std::size_t span_c = static_cast<std::size_t>(params[SPAN_C_PARAM].getValue() + 0.5f);
...

				{
					...
					but_lights[row][col] -= but_lights[row][col] / lightLambda * args.sampleTime;
		...
		{
			nob_val[row] = params[nob_map(row, nob_index)].getValue();
		...
		for (std::size_t row = 0; row < LCD_ROWS; ++row)
		{
			if (OUT_LEFT || OUT_RIGHT) outputs[lcd_val_map(row, 0)].setVoltage(lcd_val[row]);
			if (OUT_LEFT && OUT_RIGHT) outputs[lcd_val_map(row, 1)].setVoltage(lcd_val[row]);
		}
		#endif
		...
		for (std::size_t row = 0; row < NOB_ROWS; ++row)
		{
			if (OUT_LEFT || OUT_RIGHT) outputs[nob_val_map(row, 0)].setVoltage(nob_val[row]);
			if (OUT_LEFT && OUT_RIGHT) outputs[nob_val_map(row, 1)].setVoltage(nob_val[row]);
		}
		#endif
	...
	void knob_pull(std::size_t prog)
	{
		...
		{
			std::size_t prg_row = static_cast<std::size_t>(params[PRG_ROW_PARAM].getValue() + 0.5f);

			if (prg_row < LCD_ROWS)
			{
				std::size_t prg_col = static_cast<std::size_t>(params[PRG_COL_PARAM].getValue() + 0.5f);

				if (prg_col < LCD_COLS)
				{
					std::size_t prg_span   = static_cast<std::size_t>(params[PRG_SPAN_PARAM  ].getValue() + 0.5f);
					std::size_t prg_stride = static_cast<std::size_t>(params[PRG_STRIDE_PARAM].getValue() + 0.5f);

					int8_t prg_note   = static_cast<int8_t>(params[PRG_NOTE_PARAM  ].getValue() + 0.5f);
					int8_t prg_octave = static_cast<int8_t>(params[PRG_OCTAVE_PARAM].getValue() + 0.5f);
					float  prg_value  =                     params[PRG_VALUE_PARAM ].getValue();
		//			int8_t prg_gate   = static_cast<int8_t>(params[PRG_GATE_PARAM  ].getValue() + 0.5f);
	...
	void randomize_prog(std::size_t prog)
	{
		...
		for (std::size_t row = 0; row < BUT_ROWS; row++)
		{
			for (std::size_t col = 0; col < BUT_COLS; col++)
			{
				uint32_t r = random::u32() % (GATE_STATES + 1);
...
struct Display : TransparentWidget
{
	...
	{
		box  = box_;
		font = APP->window->loadFont(asset::plugin(pluginInstance, "res/fonts/lcd-solid/LCD_Solid.ttf"));
	...
	void draw_main(NVGcontext *args.vg)
	{
		...
		for (std::size_t col = 0; col < LCD_COLS; ++col)
		{
			for (std::size_t row = 0; row < LCD_ROWS; ++row)
			{
				if (old != text[row][col][0])
				{
					...
					{
						case 'p' : nvgFillColor(args.vg, nvgRGBA(0xe1, 0x02, 0x78, 0xc0)); break;
						default  : nvgFillColor(args.vg, nvgRGBA(0x28, 0xb0, 0xf3, 0xc0)); break;
					}

					old = text[row][col][0];
				}

				nvgText(args.vg, tx[col], ty[row], text[row][col] + 1, NULL);
			}
		}
	}
...
struct GtxWidget : ModuleWidget
{
	GtxWidget(GtxModule *module) : ModuleWidget(module)
	{
		...
		addParam(createParam<LEDButton>                    (but(portX[1], portY[0]), module, GtxModule::RUN_PARAM, 0.0f, 1.0f, 0.0f));
		addChild(createLight<MediumLight<GreenLight>>(l_m(portX[1], portY[0]), module, GtxModule::RUNNING_LIGHT));
		addParam(createParam<LEDButton>                    (but(portX[2], portY[0]), module, GtxModule::RESET_PARAM, 0.0f, 1.0f, 0.0f));
		addChild(createLight<MediumLight<GreenLight>>(l_m(portX[2], portY[0]), module, GtxModule::RESET_LIGHT));

		addParam(createParamGTX<KnobSnapSml>                       (Vec(portX[3], portY[0]), module, GtxModule::PROG_PARAM, 0.0f, 11.0f, 0.0f));
		addParam(createParam<CKSS>                         (tog(portX[4], portY[2]), module, GtxModule::PLAY_PARAM, 0.0f, 1.0f, 1.0f));
		addParam(createParam<CKSS>                         (tog(portX[5], portY[2]), module, GtxModule::EDIT_PARAM, 0.0f, 1.0f, 1.0f));

		addChild(createLight<SmallLight<GreenRedLight>>(l_s(portX[4] + dX - 30, portY[1] + 5 + 1), module, GtxModule::PROG_LIGHT +  0*2));  // C
		addChild(createLight<SmallLight<GreenRedLight>>(l_s(portX[4] + dX - 25, portY[1] - 5 + 1), module, GtxModule::PROG_LIGHT +  1*2));  // C#
		addChild(createLight<SmallLight<GreenRedLight>>(l_s(portX[4] + dX - 20, portY[1] + 5 + 1), module, GtxModule::PROG_LIGHT +  2*2));  // D
		addChild(createLight<SmallLight<GreenRedLight>>(l_s(portX[4] + dX - 15, portY[1] - 5 + 1), module, GtxModule::PROG_LIGHT +  3*2));  // Eb
		addChild(createLight<SmallLight<GreenRedLight>>(l_s(portX[4] + dX - 10, portY[1] + 5 + 1), module, GtxModule::PROG_LIGHT +  4*2));  // E
		addChild(createLight<SmallLight<GreenRedLight>>(l_s(portX[4] + dX     , portY[1] + 5 + 1), module, GtxModule::PROG_LIGHT +  5*2));  // F
		addChild(createLight<SmallLight<GreenRedLight>>(l_s(portX[4] + dX +  5, portY[1] - 5 + 1), module, GtxModule::PROG_LIGHT +  6*2));  // Fs
		addChild(createLight<SmallLight<GreenRedLight>>(l_s(portX[4] + dX + 10, portY[1] + 5 + 1), module, GtxModule::PROG_LIGHT +  7*2));  // G
		addChild(createLight<SmallLight<GreenRedLight>>(l_s(portX[4] + dX + 15, portY[1] - 5 + 1), module, GtxModule::PROG_LIGHT +  8*2));  // Ab
		addChild(createLight<SmallLight<GreenRedLight>>(l_s(portX[4] + dX + 20, portY[1] + 5 + 1), module, GtxModule::PROG_LIGHT +  9*2));  // A
		addChild(createLight<SmallLight<GreenRedLight>>(l_s(portX[4] + dX + 25, portY[1] - 5 + 1), module, GtxModule::PROG_LIGHT + 10*2));  // Bb
		addChild(createLight<SmallLight<GreenRedLight>>(l_s(portX[4] + dX + 30, portY[1] + 5 + 1), module, GtxModule::PROG_LIGHT + 11*2));  // B

		addParam(createParam<LEDButton>                    (but(portX[6], portY[0]), module, GtxModule::COPY_PARAM, 0.0f, 1.0f, 0.0f));
		addChild(createLight<MediumLight<GreenLight>>(l_m(portX[6], portY[0]), module, GtxModule::COPY_LIGHT));
		addParam(createParam<LEDButton>                    (but(portX[6], portY[1]), module, GtxModule::PASTE_PARAM, 0.0f, 1.0f, 0.0f));
		addChild(createLight<MediumLight<GreenLight>>(l_m(portX[6], portY[1]), module, GtxModule::PASTE_LIGHT));

		addParam(createParamGTX<KnobSnapSml>                       (Vec(portX[7], portY[0]), module, GtxModule::STEPS_PARAM, 1.0f, NOB_COLS, NOB_COLS));

		addParam(createParam<LEDButton>                    (but(portX[8], portY[0]), module, GtxModule::CLEAR_PARAM, 0.0f, 1.0f, 0.0f));
		addChild(createLight<MediumLight<GreenLight>>(l_m(portX[8], portY[0]), module, GtxModule::CLEAR_LIGHT));
		addParam(createParam<LEDButton>                    (but(portX[8], portY[1]), module, GtxModule::RANDOM_PARAM, 0.0f, 1.0f, 0.0f));
		addChild(createLight<MediumLight<GreenLight>>(l_m(portX[8], portY[1]), module, GtxModule::RANDOM_LIGHT));
...

		{
			...
			for (std::size_t row = 0; row < BUT_ROWS; ++row, ++j)
			{
				for (std::size_t col = 0; col < BUT_COLS; ++col)
				{
					addParam(createParam<LEDButton>                           (but(g_butX[col], gridY[j]), module, GtxModule::but_map(row, col), 0.0f, 1.0f, 0.0f));
					addChild(createLight<MediumLight<RedGreenBlueLight>>(l_m(g_butX[col], gridY[j]), module, GtxModule::led_map(row, col, 0)));
				}
			}
			#endif
		}
...
Model *model = createModel<GtxModule, GtxWidget>("Seq-G1-alpha1");
