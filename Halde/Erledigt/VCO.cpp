...
void VCO::process(const ProcessArgs& args) {
	oscillator.analog = params[MODE_PARAM].getValue() > 0.0f;
	oscillator.soft = params[SYNC_PARAM].getValue() <= 0.0f;
	...
	oscillator.setPulseWidth(params[PW_PARAM].getValue() + params[PWM_PARAM].getValue() * inputs[PW_INPUT].getVoltage() / 10.0f);
...
struct VCOBank : Module
{
	...
	void process(const ProcessArgs& args) override
	{
		for (std::size_t i=0; i<GTX__N; ++i)
		{
			...
			for (std::size_t p=0; p<VCO::NUM_INPUTS;  ++p) inst[i].inputs[p]  = inputs[imap(p, i)].isConnected() ? inputs[imap(p, i)] : inputs[imap(p, GTX__N)];
			for (std::size_t p=0; p<VCO::NUM_OUTPUTS; ++p) inst[i].outputs[p] = outputs[omap(p, i)];

			inst[i].step();

			for (std::size_t p=0; p<VCO::NUM_OUTPUTS; ++p) outputs[omap(p, i)].setVoltage(inst[i].outputs[p].value);
		}
	}
};
...
struct GtxWidget : ModuleWidget
{
	GtxWidget(VCOBank *module) : ModuleWidget(module)
	{
		...
		#if GTX__SAVE_SVG
		{
			PanelGen pg(asset::plugin(pluginInstance, "build/res/VCO-F1.svg"), box.size, "VCO-F1");
		...
		setPanel(APP->window->loadSvg(asset::plugin(pluginInstance, "res/VCO-F1.svg")));
		...
		addParam(createParam<CKSS>(tog(fx(-0.2), fy(-0.1)), module, VCO::MODE_PARAM, 0.0f, 1.0f, 1.0f));
		addParam(createParam<CKSS>(tog(fx(+0.2), fy(-0.1)), module, VCO::SYNC_PARAM, 0.0f, 1.0f, 1.0f));
...
Model *model = createModel<VCOBank, GtxWidget>("VCO-F1");
...
void VCO2::process(const ProcessArgs& args) {
	oscillator.analog = params[MODE_PARAM].getValue() > 0.0f;
	oscillator.soft = params[SYNC_PARAM].getValue() <= 0.0f;

	float pitchCv = params[FREQ_PARAM].getValue() + dsp::quadraticBipolar(params[FM_PARAM].getValue()) * 12.0f * inputs[FM_INPUT].getVoltage();
	...
	// Set output
	float wave = clamp(params[WAVE_PARAM].getValue() + inputs[WAVE_INPUT].getVoltage(), 0.0f, 3.0f);
	float out;
	if (wave < 1.0f)
	...
	outputs[OUT_OUTPUT].setVoltage(5.0f * out);
}
...
struct VCO2Bank : Module
{
	...
	void process(const ProcessArgs& args) override
	{
		for (std::size_t i=0; i<GTX__N; ++i)
		{
			...
			for (std::size_t p=0; p<VCO2::NUM_INPUTS;  ++p) inst[i].inputs[p]  = inputs[imap(p, i)].isConnected() ? inputs[imap(p, i)] : inputs[imap(p, GTX__N)];
			for (std::size_t p=0; p<VCO2::NUM_OUTPUTS; ++p) inst[i].outputs[p] = outputs[omap(p, i)];

			inst[i].step();

			for (std::size_t p=0; p<VCO2::NUM_OUTPUTS; ++p) outputs[omap(p, i)].setVoltage(inst[i].outputs[p].value);
		}
	}
};
...
struct GtxWidget : ModuleWidget
{
	GtxWidget(VCO2Bank *module) : ModuleWidget(module)
	{
		...
		#if GTX__SAVE_SVG
		{
			PanelGen pg(asset::plugin(pluginInstance, "build/res/VCO-F2.svg"), box.size, "VCO-F2");
		...
		setPanel(APP->window->loadSvg(asset::plugin(pluginInstance, "res/VCO-F2.svg")));
		...
		addParam(createParam<CKSS>(tog(fx(1.27), fy(-0.28)), module, VCO2::MODE_PARAM, 0.0f, 1.0f, 1.0f));
		addParam(createParam<CKSS>(tog(fx(1.27), fy(+0.28)), module, VCO2::SYNC_PARAM, 0.0f, 1.0f, 1.0f));
...
Model *model = createModel<VCO2Bank, GtxWidget>("VCO-F2");
