...
struct GtxModule : Module
{
	...
	void process(const ProcessArgs& args) override
	{
		...
		if (inputs[BLEND12_INPUT].isConnected()) blend12 *= clamp(inputs[BLEND12_INPUT].normalize(10.0f) / 10.0f, 0.0f, 1.0f);
		if (inputs[BLENDAB_INPUT].isConnected()) blendAB *= clamp(inputs[BLENDAB_INPUT].normalize(10.0f) / 10.0f, 0.0f, 1.0f);
...
Model *model = createModel<GtxModule, GtxWidget>("Fade-G2");  // right tag?
