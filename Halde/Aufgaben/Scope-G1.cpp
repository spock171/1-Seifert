...
void Scope::process(const ProcessArgs& args) {
	// Modes
...
void Scope::Voice::step(bool external, int frameCount, const Param &trig_param, const Input &x_input, const Input &trig_input) {
	...
	if (bufferIndex >= BUFFER_SIZE) {
		...
		// Reset if triggered
		float holdTime = 0.1f;
		if (resetTrigger.process(rescale(gate, trig_param.value - 0.1f, trig_param.value, 0.f, 1.f)) || (frameIndex >= args.sampleRate * holdTime)) {
...
struct Display : TransparentWidget {
	...
	void drawWaveform(NVGcontext *args.vg, float *valuesX, const Rect &b) {
		if (!valuesX)
			return;
		nvgSave(args.vg);
	...
	void drawTrig(NVGcontext *args.vg, float value, const Rect &b) {
		nvgScissor(args.vg, b.pos.x, b.pos.y, b.size.x, b.size.y);
	...
	void drawStats(NVGcontext *args.vg, Vec pos, const char *title, const Stats &stats) {
...
struct GtxWidget : ModuleWidget
{
	GtxWidget(Scope *module) : ModuleWidget(module)
	{
		...
		addParam(createParam<CKSS>  (tog(gx(3-0.22), gy(2-0.24)), module, Scope::EXTERNAL_PARAM, 0.0f,      1.0f,   1.0f));
...
Model *model = createModel<Scope, GtxWidget>("Scope-G1");
